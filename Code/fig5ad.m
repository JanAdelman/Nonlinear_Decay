...
mu_p = logspace(log10(0.001*mu_d), log10(10*mu_d), 20); % mean morphogen production rate [substance/(µm^3*s)]
...
for i = 1:length(powers)
    ...
    for p_val = 1:length(mu_p)
            ...
            % filename
            filename = [dir '/non_linear_decay_' num2str(mu_p(p_val)/mu_d/C_ref) '_' num2str(n) '_varying_p.csv'];
