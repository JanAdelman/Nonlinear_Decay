...
for i = 1:length(powers)
     ...
     if n == 4
         mu_c0 = logspace(log10(0.4), log10(4), 10) * C_ref;
     else
         mu_c0 = logspace(-2, 2, 30) * C_ref;
     end 
     ...
     for bc_val = 1:length(mu_c0)
           ...
           % to find concentrations along the domain 
            ...
            % filename
            filename_dirichlet = [dir '/c0_change/non_linear_decay_dirichlet_bc_' num2str(mu_c0(bc_val)/C_ref) '_' num2str(n) '.csv'];
            ...
            interp_readout_positions_5_cells_dirichlet(bc_val, :) = [mu_c0(bc_val)/C_ref, interp_std_dirichlet(1), interp_SE_dirichelt(1)];
            interp_readout_positions_50_cells_dirichlet(bc_val, :) = [mu_c0(bc_val)/C_ref, interp_std_dirichlet(2), interp_SE_dirichelt(2)];
            interp_readout_positions_150_cells_dirichlet(bc_val, :) = [mu_c0(bc_val)/C_ref, interp_std_dirichlet(3), interp_SE_dirichelt(3)];

     end
   
     filename_dirichlet_interp_5 = [dir '/c0_change_' num2str(n) '_readout_five_cells.csv'];
     filename_dirichlet_interp_50 = [dir '/c0_change_' num2str(n) '_readout_fifty_cells.csv'];
     filename_dirichlet_interp_150 = [dir '/c0_change_' num2str(n) '_readout_hundred_fifty_cells.csv'];
