...
CV_p = logspace(log10(0.01), log10(1), 20); % define variaility in production rate p 
...
if not(isfolder('production_var'))
    mkdir('production_var')
end
...
for i = 1:length(powers)
    ...
    % get domain 
    [l_s, l_p] = helper_functions.build_domain(LS, LP(1), diameter, CV_area);
...
for i = 1:length(powers)
    ...
    for p_var = 1:length(CV_p)
            ...
            % filename
            filename = ['production_var/non_linear_decay_' num2str(CV_p(p_var)) '_' num2str(n) '_varying_p.csv'];
     ...
     filename_interp_5 = ['production_var/non_linear_decay_'  num2str(n) '_readout_five_cells_varying_p.csv'];
     filename_interp_50 = ['production_var/non_linear_decay_'  num2str(n) '_readout_fifty_cells_varying_p.csv'];
     filename_interp_150 = ['production_var/non_linear_decay_'  num2str(n) '_readout_hundred_fifty_cells_varying_p.csv'];
