...
if not(isfolder('bc_change/flux_change/'))
    mkdir('bc_change/flux_change/')
end

%% Dirichlet boundary conditions c(0) = 1 & Flux boundary conditions at x=0
...
for i = 1:length(powers)
     ...
     % n=4
     if n == 4
         mu_j = logspace(log10(0.00001), log10(0.01), 20)/j_ref; % average flux 
     else
         mu_j = logspace(log10(0.00001), log10(1), 30)/j_ref; 
     end 
     ...
     for bc_val = 1:length(mu_j)
            ...
            % to find concentrations along the domain 
                ...
                sol_flux = solve_ode_flux_bc(x0, nc, 0, 0, n, tol, mu_d, mu_D, mu_j(bc_val));
            ...
            % filename
            filename_flux = ['bc_change/flux_change/non_linear_decay_flux_bc_' num2str(mu_j(bc_val)) '_' num2str(n) '.csv'];
            ...
            for j = 1:nruns
                    ...
                    sol_flux = solve_ode_flux_bc(x0, nc, CV, CV_bc, n, tol, mu_d, mu_D, mu_j(bc_val));
     ...
     filename_flux_interp_5 = ['bc_change/flux_change_'   num2str(n) '_readout_five_cells.csv'];
     filename_flux_interp_50 = ['bc_change/flux_change_'   num2str(n) '_readout_fifty_cells.csv'];
     filename_flux_interp_150 = ['bc_change/flux_change_'   num2str(n) '_readout_hundred_fifty_cells.csv'];
...
function sol_flux = solve_ode_flux_bc(x0, nc, CV, CV_bc, n, tol, mu_d, mu_D, j)
    ...
    dir_j  = random(helper_functions.logndist(j, CV_bc), 1, 1);
