...
% add noise
CV = 0.3;
CV_A = logspace(-2, 1, 20);

% loop over all n
for i = 1:length(powers)

    interp_std = NaN(length(CV_A), 3);
    interp_SE = NaN(length(CV_A), 3);

    filename_readout_positions = [dir '/non_linear_decay_readout_positions_' num2str(n) '.csv'];
    
    %loop over the area variabilities 
    for area_var = 1:length(CV_A)
        
        % get the power of the nonlinear decay 
        n = powers(i);

        % filename
        filename = [dir '/non_linear_decay_' num2str(CV_A(area_var)) '_' num2str(n) '.csv'];
        ...
        for j = 1:nruns
                ...
                [l_s, l_p] = helper_functions.build_domain(LS, LP, diameter, CV_A(area_var));
    ...
    writetable(table(CV_A', interp_std(:, 1), interp_SE(:, 1), interp_std(:, 2), interp_SE(:, 2), interp_std(:, 3), interp_SE(:, 3), ...
    'VariableNames', {'CV_A', 'std_five_cells', 'SE_five_cells', 'std_fifty_cells', 'SE_fifty_cells', 'std_hundred_fifty_cells','SE_hundred_fifty_cells'}), ...
