...
mu_lambda_not_scalled = 20;% gradient decay length 
mu_D = 0.033; % mean morphogen diffusion constant [µm^2/s]
mu_d = mu_D/mu_lambda_not_scalled^2; % mean morphogen degradation rate [1/s]
...
diameters = linspace(mu_lambda_not_scalled/40, 2*mu_lambda_not_scalled, 20); % define a range of varying diameters
powers = [1,2,4]; 

% analytical deterministic solution
C = @(x) mu_p/mu_d * ((x<0) .* (1-cosh(x/mu_lambda_not_scalled)) + sinh(LS/mu_lambda_not_scalled) / sinh((LS+LP)/mu_lambda_not_scalled) * cosh((LP-x)/mu_lambda_not_scalled));
...
if not(isfolder('non_linear_decay_var_diam'))
    mkdir('non_linear_decay_var_diam')
end
...
mu_lambda = mu_lambda_not_scalled/(1 - 0.011 * CV + 1.355 * CV^2 - 0.179 * CV^3 + 0.0077 * CV^4)^-0.357;
...
for i = 1:length(powers)
    ...
    % to find concentrations along the domain 
        ...
        K = helper_functions.get_readout_conc_non_linear(readout_position, n, C_0(i), mu_lambda_not_scalled, c_ref);
    end 
    ...
    for diam = 1:length(diameters)
            ...
            for j = 1:nruns
                    ...
                    [l_s, l_p] = helper_functions.build_domain(LS, LP, diameters(diam), CV_area);
    ...
    filename_five_lambda = ['non_linear_decay_var_diam/non_linear_decay_varying_diameters_' num2str(n)  '_'  num2str(readout_pos_name(1)) '.csv'];
    filename_ten_lambda = ['non_linear_decay_var_diam/non_linear_decay_varying_diameters_' num2str(n)  '_'  num2str(readout_pos_name(2)) '.csv'];
    filename_fifteen_lambda = ['non_linear_decay_var_diam/non_linear_decay_varying_diameters_' num2str(n)  '_'  num2str(readout_pos_name(3)) '.csv'];
