...
if not(isfolder('non_linear_decay_kin_param'))
    mkdir('non_linear_decay_kin_param')
end
...
for k = 1:numel(kin_params)
    ...
    for i = 1:length(powers)
        ...
        % filename
        filename = ['non_linear_decay_kin_param/non_linear_decay_'  kin_params{k}  '_' num2str(n)  '.csv'];
        
        if kin_params{k} == 'D'
            filename = ['non_linear_decay_kin_param/non_linear_decay_Diff_' num2str(n) '.csv'];
        end
        ...
        for j = 1:nruns
                ...
                % find the place where the threshold concentration is reached                 
